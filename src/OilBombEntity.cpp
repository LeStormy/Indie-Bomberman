...
OilBombEntity::OilBombEntity(Tile &tile, APlayer *player)
{
  _timer = 2.0;
  _tile = &tile;
  _player = player;
  _range = _player->getBombRange();
  _type = Indie::OILBOMB_ENTITY;
  _mov.reset(new MovementManager(this));
  init(Indie::OILBOMB_ENTITY);
...
OilBombEntity::~OilBombEntity()
{
  ...
  Core::Sound.playSound(AUDIO("BombLiquid"));
  for (uint16_t i = _range; i > 0 && (tmp = tmp->getMapHandler()->getTile<Indie::UP_DIRECTION>(tmp))
    ...
    if (!tmp->isEntitiesExisting({Indie::BREAKABLE_ENTITY, Indie::MAGNET_ENTITY}))
      tmp->addEntity(Indie::OIL_ENTITY, _player);
  ...
  _tile->addEntity(Indie::OIL_ENTITY, _player);
  _tile->setReachable(true);
  _player->incBombCnt();
  _node->remove();
}

void			OilBombEntity::update()
{
  ...
  if ((_timer -= Core::frameRate) <= 0)
    _tile->deleteEntities({Indie::OILBOMB_ENTITY});
